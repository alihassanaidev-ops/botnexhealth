...
Two resources over the V-4 data model:
  * ``/outbound-voice/profiles`` — clinic-readable, sanitized outbound profile
    options for workflow selection. Technical writes are Super Admin-only; clinics
    never receive Retell/Vapi identifiers through these read endpoints. A location can
    have multiple active profiles, with at most one active profile per purpose.
...
# Managing technical outbound voice profiles is a platform-admin action. Clinic
# workflow screens can only read sanitized profile names.
...
class OutboundVoiceProfileResponse(BaseModel):
    ...
    @classmethod
    def from_model(cls, p: Any, *, include_sensitive: bool = True) -> "OutboundVoiceProfileResponse":
        return cls(
            id=str(p.id),
            institution_id=str(p.institution_id),
            location_id=str(p.location_id),
            retell_agent_id=p.retell_agent_id if include_sensitive else None,
            retell_from_number=p.retell_from_number if include_sensitive else None,
            retell_llm_id=p.retell_llm_id if include_sensitive else None,
...
_ACTIVE_PROFILE_CONFLICT = "An active outbound-voice profile with this purpose already exists for this location"
...
async def create_profile(
    ...
    async with get_db_session() as session:
        ...
        profile = OutboundVoiceProfile(
            ...
            purpose=_normalize_purpose(data.purpose),
        ...
        try:
            ...
            # Partial-unique index: at most one active profile per location/purpose.
...
async def list_profiles(
    ...
    async with get_db_session() as session:
        ...
        if purpose is not None:
            query = query.where(OutboundVoiceProfile.purpose == _normalize_purpose(purpose))
        query = query.order_by(OutboundVoiceProfile.display_name.asc().nulls_last(), OutboundVoiceProfile.created_at.desc())
        profiles = (await session.execute(query)).scalars().all()
    return [OutboundVoiceProfileResponse.from_model(p, include_sensitive=False) for p in profiles]
...
async def get_profile(
    ...
    async with get_db_session() as session:
        ...
        return OutboundVoiceProfileResponse.from_model(profile, include_sensitive=False)
...
async def update_profile(
    ...
    async with get_db_session() as session:
        ...
        for field, value in data.model_dump(exclude_unset=True).items():
            if field == "purpose":
                value = _normalize_purpose(value)
