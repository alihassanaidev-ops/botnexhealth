...
* **Write + wiring:** `suppress_voice_optout` records a location-scoped revoked
  VOICE consent. Other channels remain contactable unless they have their own
  opt-out, and the compliance gate blocks later voice dispatches.
"""
...
# Short, non-PHI reason code stamped on the consent row.
...
async def suppress_voice_optout(
    *,
    ...
    """Write a location-scoped VOICE revocation for a spoken opt-out. Best-effort.
    ...
    if not phone:
        logger.warning(
            "voice opt-out: no phone to suppress institution=%s call=%s",
            institution_id,
            call_id,
        )
        return False
    try:
        async with get_system_db_session(
            ...
            await SmsComplianceService(session).record_consent(
        ...
        logger.info(
            "voice opt-out: location voice revocation written institution=%s location=%s call=%s",
            institution_id,
            location_id,
            call_id,
        )
        ...
        logger.error(
            "voice opt-out: failed to write revocation institution=%s call=%s error=%s",
            institution_id,
            call_id,
            exc,
        )
