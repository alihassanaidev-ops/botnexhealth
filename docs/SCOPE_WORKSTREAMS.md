...
| **12** · Generate the three link types — *token layer* | Campaign engine core | `797063d` | Signed, run-scoped, expiring tokens following the existing unsubscribe pattern; action and expiry both inside the signature; `Referrer-Policy: no-referrer` for whatever serves them. **Remaining: the three landing endpoints** that consume a token and perform the action |
| **16** · Cross-channel suppression | Campaign engine core | `46a3a9f` | Moved from "how the campaign was drawn" to an engine rule in the compliance gate, checked before quiet hours. Opt-out via `ComplianceMetadata.continue_after_response`. Verified safe for both live campaigns — they hold only a voice attempt ladder, no post-response sends |
...
| **12** ◐ | Generate the three link types — **token layer done, `797063d`** | 3d | **Currently unsafe.** Six templates already use the placeholder; nothing generates the value. Needs signed, expiring, per-run links plus patient-facing pages |
